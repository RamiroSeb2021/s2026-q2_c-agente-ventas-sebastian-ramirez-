...
**Goal**: Produce/update SDD spec and design artifacts for `openspec/changes/streamlit-sales-query-agent` using `eci_genai` as reference context, selecting the next-slice architecture for a Streamlit sales query agent over `ventas` with Bedrock, LangGraph/LangChain-compatible orchestration, SQL validation, and MCP SQL access.

**Context/evidence**:
- Current repo has a verified Slice 1 Python/uv scaffold with `pyproject.toml`, `uv.lock`, `scripts/seed_database.py`, `tests/test_seed_database.py`, and passing `uv run pytest`; it still has no Streamlit app, Dockerfile, compose file, MCP connector, Bedrock integration, or agent flow.
...
- Remaining proposal work requires SQLite MCP access, Bedrock SQL generation, single-statement read-only SQL validation, generated SQL transparency, Streamlit UI, and table output; deterministic SQLite seeding is already implemented.
