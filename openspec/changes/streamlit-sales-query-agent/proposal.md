...
The repository is currently initialized with documentation and configuration guidelines, but lacks a runnable application for the Agentic AI sales analysis assignment. To incrementally build the solution and minimize risk, we need an initial vertical slice that proves the core assumptions: using Amazon Bedrock for semantic intent and SQL generation, querying a local SQL database through MCP, and rendering the result in a Streamlit UI, all managed via `uv`.
...
- **Dependency Management**: Initialize the Python project configuration with `uv` using `pyproject.toml` and generate/update `uv.lock`.
- **Data Generation**: Create a deterministic Python seed script (for example `scripts/seed_database.py`) that uses a fixed random seed to simulate sales records matching the domain model (`id`, `vendedor`, `sede`, `producto`, `cantidad`, `precio`, `fecha`).
- **Generated Data Artifacts**: Generate `data/ventas.csv` and a local SQLite database (`data/ventas.db` or equivalent) from the seed script. Both files are reproducible runtime artifacts, not source of truth.
...
scripts/seed_database.py    deterministic fixed-seed data generator
   ↓ creates/refreshes
data/ventas.csv             generated simulated sales data
   ↓ loads
data/ventas.db              generated local SQLite database
   ↓ exposed through
...
1. User prepares dependencies with `uv` after `pyproject.toml` exists.
...
3. Docker Compose or the local setup runs the deterministic seed script to generate `data/ventas.csv` and `data/ventas.db` if needed.
