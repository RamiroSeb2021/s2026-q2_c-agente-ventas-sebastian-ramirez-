...
- [ ] **GREEN: implement MCP wrapper** in `src/mcp_client.py` and any thin adapter needed to connect to the chosen fixed/configured SQLite MCP server/connector against `data/sales.db`; expose `check_connection()` plus `execute_readonly_sales_query(sql)` only.
...
- [ ] **Validate Slice 3** by running, once files exist: `uv run pytest tests/test_mcp_client.py tests/test_graph.py` and a focused manual connector smoke check against a generated `data/sales.db`.
